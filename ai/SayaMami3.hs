...
mamimami = do
  ...
  lstToKill <- mapM (\i -> do
                   v     <- getVital False i
                   return (if v > 0 then (False, v * (1 + numCost (255-i))) else (True, 0),i) ) $ [0..255] 
  let ((isTKDead, _),iToKill0) = (minimum lstToKill)
  let iToKill = if not isTKDead then 255 - iToKill0 else 0

  liveList <- mapM (\i -> do
                   v     <- getVital True i
                   return ((v, -(numCost i)), i)) $ ([8,16,32,64,128]) -- ([8..153]++[155..255])
  liveList2 <- mapM (\i -> do
                   v     <- getVital True i
                   return ((v, -(numCost i)), i)) $ ([8..153]++[155..255])

  let ixLive = if length liveList >= 1
                 then snd $ maximum liveList2
                 else 154
  ...
  (maxf, maxname) <- (iter l0 (-2000000000) "Idle" (\_ -> do
                                                            revive ixLive
                                                            v  <- getVital True ixLive
                                                            ve <- getVital False (255-iToKill)
                                                            mglim <- lift $ readIORef magicalLimit
                                                            if v < 10 || ve < 10 
                                                            then do
                                                              num ixLive iToKill
                                                              Dec $> ixLive
                                                                 ...
                                                                 then do
                                                                   lift $ writeIORef magicalLimit 55000
                                                                   clear ixLive
                                                                   ixLive $< Help
                                                                   num 0 ixLive
                                                                   apply0 ixLive
                                                                   apply0 ixLive
                                                                   let atk = getEasyInt v
                                                                   num 0 atk
                                                                   lprint $ "HELP " ++  show ixLive ++ " " ++  show atk
                                                                   apply0 ixLive
                                                                 else do
                                                                   num ixLive ixLive
                                                                   Attack $> ixLive
                                                                   num 0 iToKill
                                                                   apply0 ixLive 
                                                                   let atk = getEasyInt (min v (ve*10`div`8))
                                                                   lprint $ "ATAK " ++ show iToKill ++ "<-"++ show ixLive ++ " " ++  show atk  ++ " / " ++ show ve ++ " " 
                                                                           ++ show v ++ " " 
                                                                           ++ show (min v (ve*10`div`8))
                                                                   f <- getField True ixLive
                                                                   lprint f
                                                                   num 0 atk
                                                                   apply0 ixLive 
                                                   ))
...
magicalLimit :: IORef Int
magicalLimit = unsafePerformIO $ newIORef 10000
{-# NOINLINE magicalLimit #-}
...
mamisanMain = do
  ...
  if (f/=weapon) then prepareMagicalBullet 1 2
    ...
    let range i = [i .. min 255 (i+124)]
        ...
        withScore i = ((deaths i, damages i,- numCost i), i)
